...
class GraphicsView(QtWidgets.QGraphicsView):
    ...
    Used to draw polygons, draw junction points, and translate the viewing plane
    """
    def mousePressEvent(self, event):
    ...
    def mouseMoveEvent(self, event):
        if self.drawPolygon_Bool == False:
            if self.drawJunction_Bool == False:
                if (self.inBounds(event.x(), event.y())):
                    x_disp = self.startX - event.x()
                    y_disp = self.startY - event.y()
                    if event.buttons() & Qt.LeftButton:
...
class MainWindow(QMainWindow):
    ...
    def resetPolygon(self):
        self.view.scene.removeItem(self.view.polygon)
        self.view.polygon = QtWidgets.QGraphicsPolygonItem(self.view.pixmap)
        self.view.polygon.setPen(QtGui.QPen(QtCore.Qt.red, 3, QtCore.Qt.SolidLine))
        self.view.polygon.setBrush(QtGui.QBrush(QtCore.Qt.red))
        ...
        if len(self.glWidget.annotations)>0: # if first branch, do not need a junction point
            ...
            if next:
                ...
                if ok:
                    ...
                    self.hierarchy[parentBranch].append(createdBranch)
    ...
    def removeBranch_button_clicked(self):
        ...
        if ok:
            ...
            for childBranch in self.hierarchy[currBranch]:
        ...
        if next:
            ...
            if ok:
                ...
                type, final = QInputDialog.getItem(self, 'Type of Annotation',
                                                        'Select Annotation:', ["polygon", "junction point"])
    ...
    def finish_button_clicked(self):
        ...
        if self.type == "polygon":
            self.view.drawPolygon_Bool = False
            poly = self.make_polygons(self.view.polygon)
            if len(poly)>0:
                center = self.getCenter(poly)
        ...
        if self.type == "junction":
            if len(self.view.junction)>0:
                ...
                self.view.polygons_planes[self.glWidget.currBranch].insert(self.glWidget.currPolygon, plane)
                self.view.junction = []
        print("annotations",self.glWidget.annotations)
    ...
    def save(self):
        ...
        if name != "":
            ...
            for branch in branches:
                for i in range (len(self.hierarchy[b])):
                    f.write(str(self.hierarchy[b][i]) + "\n")
                f.write("next"+"\n")
    ...
    def open(self):
        ...
        # for each branch, rgb colors and num polygons in that branch
            for i in range(numBranches):
                ...
                self.hierarchy.append(branchChildren)
                print("curr", curr)
                r = int(f.readline())
                g = int(f.readline())
                b = int(f.readline())
